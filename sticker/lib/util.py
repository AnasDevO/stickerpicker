...
def add_thumbnails(stickers: List[matrix.StickerInfo], stickers_data: Dict[str, bytes], output_dir: str) -> None:
    ...
    for sticker in stickers:
        # This line is problematic - _convert_sticker returns a tuple of (bytes, str, int, int)
        # But we're not unpacking the result properly
        data, mimetype, width, height = _convert_sticker(stickers_data[sticker["url"]])

        name = sticker["url"].split("/")[-1]
        thumbnail_path = thumbnails / name
        thumbnail_path.write_bytes(data)  # Now we use only the bytes part of the return value
